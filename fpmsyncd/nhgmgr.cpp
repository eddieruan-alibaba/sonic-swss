...
int NHGMgr::updateExistingNHGFull(NextHopGroupFull nhg, uint8_t af) {
    ...
    if(previousKey != entry->getSonicNHGObjectKey() && entry->needCreateSonicObject()){
        ...
        /* Key unchanged but checkNeedUpdate returned true (e.g. PIC backwalk
         * had rewritten APPDB with a reduced nexthop set, and zebra has now
         * resent the original NHGFULL on recovery). The fvVector was just
         * regenerated by setEntry; push it so APPDB matches RIB. writeToDB
         * dedups via m_last_appdb_fields. */
        if (entry->getSonicObjID() != 0) {
            ret = m_rib_nhg_table->writeToDB(entry);
            if (ret != 0) {
                SWSS_LOG_ERROR("Failed to refresh APPDB for %d", nhg.id);
                return ret;
            }
            SWSS_LOG_NOTICE("Refresh sonic NHG fields for %d, sonic id %d",
                            nhg.id, entry->getSonicObjID());
        }
    }
