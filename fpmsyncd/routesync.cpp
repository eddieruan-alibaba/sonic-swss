...
bool RouteSync::getSrv6VpnRouteNextHop(struct nlmsghdr *h, int received_bytes,
                               ...
                               uint32_t &nhg_id)
{
    SWSS_LOG_INFO("getSrv6VpnRouteNextHop begins");
    uint16_t encap = 0;
...
void RouteSync::onSrv6VpnRouteMsg(struct nlmsghdr *h, int len)
{
    ...
    else if (nlmsg_type == RTM_NEWSRV6VPNROUTE)
    {
        ...
        /* We do not use 'pic_id' from zebra as pic_context_id in rib/fib.
         * Instead, we use nhg_id and nhg_received_id to get corresponding entries
         * directly.
         */
        ...
        if(nhg_received_entry->isSingleNexthop())
        {
            ...
            fvVector.push_back(intf);
            /* In RIB/FIB we do not write weights for SRv6 VPN case */
            // if(!weights.empty())
            // {
            //     FieldValueTuple wg("weight", nhg_received_entry->getWeightStr().c_str());
            ...
            /* In RIB/FIB, we have already have the PIC Context when processing the NextHopGroupFull,
             * so we do not need to update the nexthop memory table here.
             */
    ...
    if(nhg_id)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                FieldValueTuple nhg_fv("nexthop_group", nhg_id_key.c_str());
                fvVector.push_back(nhg_fv);
...
/*
 * onNhtEventMsg() handles NHT (Next Hop Tracking) event messages from zebra.
 * When a tracked nexthop becomes unreachable (curr_resolved_nhg_id == 0),
 * triggers the backwalk-based quick fixup to prune failed paths.
 */
void RouteSync::onNhtEventMsg(struct nlmsghdr *h, int len)
{
    ...
    if (!tb[NHA_JSON_STR])
    {
        SWSS_LOG_ERROR("NHT event without JSON string received from zebra");
        return;
    }
    ...
    try {
        ...
        /* Phase 1: Only handle nexthop becoming unreachable */
        ...
        /* Extract nexthop address from rnh_prefix by stripping /mask */
        ...
        SWSS_LOG_NOTICE("NHT event: nexthop %s unreachable, prev_resolved_nhg_id=%u, triggering quick fixup",
