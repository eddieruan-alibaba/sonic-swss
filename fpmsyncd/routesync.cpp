...
void RouteSync::onSrv6VpnRouteMsg(struct nlmsghdr *h, int len)
{
    ...
    else if (nlmsg_type == RTM_NEWSRV6VPNROUTE)
    {
...

        {
            ...
            SWSS_LOG_INFO("onSrv6VpnRouteMsg: nhg_received %d, filling route table %s with pic_context_id: %d, nexthop_group: %d",
...
void RouteSync::onNhtEventMsg(struct nlmsghdr *h, int len)
{
    ...
    try {
        ...
        if (nht_event.prev_resolved_nhg_id == 0) {
            SWSS_LOG_DEBUG("NHT event for %s: prev_resolved_nhg_id=%u, skipping (not Phase 1)",
                nht_event.rnh_prefix.c_str(), nht_event.curr_resolved_nhg_id);
            return;
        }
